...
-   **[capture clause]** 标记了 lambda 的声明以及如何捕获局部作用域参数（按值捕获、按引用捕获等）。例如，`[=]` 表示捕获所有外部变量通过值，`[&]` 表示捕获所有外部变量通过引用。
-   **parameters** 是 lambda 的普通函数参数（可选的）。
-   **body** 是一个普通的函数体。

等号右侧的表达式是 lambda 表达式，通过该表达式创建的运行时对象 x 是闭包 （closure）。


<!--stackedit_data:
eyJoaXN0b3J5IjpbLTY4NTQ0NjI0LC0xNDg5ODIxOTEzLC0xNT
k5OTUxMjQ1LC0xODYxNDgzNTQ0LDEwMzQxOTIzMTUsLTE4NDEy
MDM3MjMsMTUwNzI3NDQ5MiwxODU0MDQ3NjAxXX0=
-->