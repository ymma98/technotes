...
在 C++ 中，对于常量（`const`）和引用（`&`）类型的数据成员，由于它们在创建后不能被修改（对于常量）或重新绑定（对于引用），因此必须在对象的构造期间立即初始化。这可以通过以下两种方式之一完成：

1. **使用初始化列表**：
   - 在构造函数定义中，初始化列表直接跟在构造函数的参数列表之后，使用冒号 `:` 开始。这个列表中的每个初始化器都直接初始化一个数据成员。
   - 初始化列表确保成员变量在构造函数体执行之前就已经被初始化，这对于 `const` 成员和引用成员尤其重要。

2. **使用类内初始化器**（C++11 引入）：
   - 类内初始化器允许在类定义中直接为数据成员提供初始值。这种方式可以使用等号 `=` 或大括号 `{}` 进行初始化。
   - 对于常量和引用类型的成员，这种初始化方式提供了一种便捷的方法来设置默认值，尤其是当类有多个构造函数时。

### 示例解释

在给定的代码示例中：
- 成员 `y` 是一个常量字符，必须在构造期间初始化，这里通过构造函数的初始化列表完成：`y('a')`。
- 成员 `z` 是一个对整型的引用，同样需要在构造期间通过初始化列表进行初始化：`z(x)`。
- 成员 `v` 是一个引用，通过等号初始化器直接在类内与成员 `x` 绑定。
- 成员 `w` 是一个常量整数，使用大括号初始化器直接在类定义中初始化为 `4`。

这些初始化方式展示了 C++11 增强的类成员初始化功能，使得代码更加灵活且易于管理。

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyNjQ5NTMzODAsMTc2ODA1MTI4MCwxNT
