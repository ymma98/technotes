...
-   重解释转换（reinterpret_cast）
	...
	`reinterpret_cast<T&>(v)` 等于 `*((T))`
 重解释转换<T*>(v) 等同于 (T*) v 重解释转换<T&>(v) 等同于 _((T_)&v) 常量转换和重解释转换不编译成任何CPU指令。


<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQxMjQxNzkzNiwtNzEyMTA4NjMwLDE0OT
M1MzYzODEsMTMzOTI4Nzk3Nyw5NjA5NzU1ODddfQ==
-->