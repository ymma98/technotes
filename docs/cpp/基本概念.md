...
* 面临的问题: 虽然从技术上来说 `color` 和 `fruit` 是相等的（因为它们的整数值相同），但从逻辑上将颜色与水果进行比较并没有任何意义。这是使用枚举时可能遇到的一个问题，尤其是当枚举类型用于不相关的领域时。这种行为可能导致代码中出现难以发现的逻辑错误。
```cpp
// 定义颜色枚举类型
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjI3MTQ4MDUzLDEzMTgxNDE1MTEsMTY2MD
E0MTc0MywtODQ1ODUwMjg1LDEwMzUwMDk2NzgsMTQ3NjE3ODAw
MCwtMTIzODU3MDAxMSwtMjEwMjU5OTUwMSwtMTY0ODk3NjQ0MC
w5NjAwOTQ1OTgsLTE1MDgxMjU4ODAsMjAyMTMwMTIyOSwtNjcy
NzA0MDIwLC0zMTIyOTI1NDgsLTE2MzU5OTMyNTIsLTE0MTIzNT
Y1ODgsLTExOTkyNjM4NzYsOTQwNzc2ODgwLC00NDMyNzk5MDcs
LTExODM0Nzc1MjhdfQ==
-->