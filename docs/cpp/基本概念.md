...
![输入图片说明](./imgs/2024-07/s93ygFszdNevNqpj.png =700x400)
...
注: \* 4 bytes on Windows64 systems, $\dagger$ signed/unsigned, two-complement from C++11 (二进制补码（Two's complement）是一种整数的二进制表示方法，广泛用于表示有符号整数。在二进制补码表示中，正数的表示与其无符号形式相同（符号位为 0），而负数是通过取其正数值的二进制反码（所有二进制位取反）然后加一得到的。在二进制补码表示中，正数的表示与其无符号形式相同（符号位为 0），而负数是通过取其正数值的二进制反码（所有二进制位取反）然后加一得到的。)
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTY3MjcwNDAyMCwtMzEyMjkyNTQ4LC0xNj
M1OTkzMjUyLC0xNDEyMzU2NTg4LC0xMTk5MjYzODc2LDk0MDc3
Njg4MCwtNDQzMjc5OTA3LC0xMTgzNDc3NTI4LC0xMTg4NDgyMj
EyLDE5NTk5MzIyNDcsLTIzMDU5NDg5LDEzNjk3NTU2LC04NjYy
MzM2MTMsNDU2OTE5MTg3LDE0NzQwMjYwMjddfQ==
-->