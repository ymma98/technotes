...
实体 (`entity`) 是一个值 value、对象 object、引用 reference、函数 function、枚举器 enumerator、类型 type、类成员或模板。标识符和用户定义的操作符是用来指代实体的名称。实体也包括表达式的结果。预处理器宏不是 C++ 的实体。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTUxNjQ1NzU2NSwxMzE4MTQxNTExLDE2Nj
