...
- floating type $\otimes$ integer type $\rightarrow$ floating type
  - 当一个浮点类型和整数类型一起进行运算时，整数会被提升为对应的浮点类型，确保运算结果为浮点类型。

### (B) Implicit Integer Promotion
- **Definition**: `small integral type` := any signed/unsigned integral type smaller than `int`
...
- **Rule**: `small type \otimes large type \rightarrow large type`
  - 在两个不同大小类型的数据进行运算时，小类型数据会被提升到大类型数据的大小，以匹配数据类型的宽度。

### (D) Sign Promotion
- **Rule**: `signed type \otimes unsigned type \rightarrow unsigned type`
  - 当有符号类型和无符号类型一起运算时，为避免负值的意外产生，有符号类型会被转换为相应的无符号类型。

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2NzUzNjA3NjQsOTYwMDk0NTk4LC0xNT
A4MTI1ODgwLDIwMjEzMDEyMjksLTY3MjcwNDAyMCwtMzEyMjky
NTQ4LC0xNjM1OTkzMjUyLC0xNDEyMzU2NTg4LC0xMTk5MjYzOD
c2LDk0MDc3Njg4MCwtNDQzMjc5OTA3LC0xMTgzNDc3NTI4LC0x
MTg4NDgyMjEyLDE5NTk5MzIyNDcsLTIzMDU5NDg5LDEzNjk3NT
U2LC04NjYyMzM2MTMsNDU2OTE5MTg3LDE0NzQwMjYwMjddfQ==
