...
*  Sign Promotion
  ...
  - 当有符号类型和无符号类型一起运算时，为避免负值的意外产生，有符号类型会被转换为相应的无符号类型。这种转换是因为无符号类型有更宽的正数范围，而编译器尝试保证数值不会因为类型转换而变成负数。此时有符号的负数，比如 -2, 会转换为很大的无符号zheng'shu

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE0NDU1MTQyMDYsOTYwMDk0NTk4LC0xNT
