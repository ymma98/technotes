...
#  `-f`: 让 ssh 命令在后台运行。
#  `-L 127.0.0.1:1233:222.20.94.38:22`: 设置本地端口转发。这告诉 `ssh` 监听本地机器上的 1233 端口，并将所有通过这个端口的数据转发到 `222.20.94.38` 的 22 端口。`222.20.94.38` 是最终目标服务器的 IP 地址，22 是 SSH 服务的标准端口。
ssh -f -L 127.0.0.1:1233:ip_B:22 ymma@211.67.27.88 sleep 10; rsync -auvzP -e 'ssh -p 1233' prj_compression/compress14_paper_20220521/paper_referee_NF_20221119 ymma@127.0.0.1:~
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTgzMjIyMTExNCwtMTk3MzAyMjg0MF19
-->