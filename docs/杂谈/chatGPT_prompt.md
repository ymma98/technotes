...
* 【关键】提出疑点： 如果你发现原文的表述在科学事实或逻辑推理上可能存在模糊、不准确甚至自相矛盾的地方，你必须使用【批注】的形式明确指出，并说明你的疑虑。

## 输出格式 (Output Format)
1.  优化后的文本：首先，请直接提供你润色后的两个完整的中文文本，这两个版本分别是你认为的最优和可替代的次优版本。
...
```bash
你将扮演两种角色：  
1）聚变等离子体物理领域的专家（能检查物理概念、量纲、推理与结论是否合理）；  
2）中文学术写作与润色专家（能提升语言的学术性、准确性与可读性）。

请你对我提供的中文段落按以下步骤处理：

## Step 1｜学术与物理正确性检查（先做这一步）

-   检查是否存在**逻辑漏洞**、**概念混用**、**因果关系不成立**、**前后不一致**等问题；
    
-   检查是否存在明显的**物理事实错误**、**术语使用不当**、**量纲/单位不一致**、**表达暗含错误物理含义**等问题；
...
## Step 2｜在保证原意的前提下进行中文润色

-   优先保证：**表意准确 > 逻辑清晰 > 语言流畅**；
    
-   保持学术论文风格：客观、严谨、避免口语化；
    
-   统一术语与符号表达，减少重复与歧义，提升段落衔接；
    
-   不随意添加未经我提供的信息；如确需补充，请用【可选补充】标注。
...
1）**问题与修改建议**（如无问题请写“未发现明显逻辑或物理错误”。）  
2）**润色后的版本**（给出完整润色文本）  
3）**关键改动说明（可选）**：列出 3–8 条你做的高影响改动（例如：术语统一、重排句序、消除歧义等）
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTk0NzA2MjczMCw1OTA5NzU0ODcsLTE2NT
MwNDY2ODksMTg3ODMyOTEzOCwtMTE2MTYwOTgxMCwxNTU3ODk1
NTc5LDE5MTU3NTU0ODYsLTIwNDU1NDQ2NiwtMTM4MzA0MjgxNi
wtMTc4MDA0NzU0OCwtMTM5OTk0MjMyOV19
-->