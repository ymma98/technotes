...
在这里，我们可以将 $u^{n-1}(x)$ 视为已知数据，因为它通常已被计算出来。现在，用
...
总之，我们要处理的是在两个不同网格上定义的形函数之间进行积分的情况。这是可行的，并在 \texttt{step-28} 中有相应示例，但用“尴尬”来形容整个过程是最贴切的。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTY0NDc4NzMxMywtNDM2MTU5MzEzLC0xOT
