...
在边界线附近的一个点, 其值可以根据边界线上的值确定 (using Taylor's series):

$$
\psi(x, y) = \psi(\xi, \eta) + \left[ (x - \xi) \frac{\partial \psi}{\partial x} + (y - \eta) \frac{\partial \psi}{\partial y} \right] 
+ \frac{1}{2} \left[ (x - \xi)^2 \frac{\partial^2 \psi}{\partial x^2} 
+ 2(x - \xi)(y - \eta) \frac{\partial^2 \psi}{\partial x \partial y} 
+ (y - \eta)^2 \frac{\partial^2 \psi}{\partial y^2} \right] + \dots
$$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA2OTQ2ODM2Niw2MjUzMjQ5MDEsLTE2OT
c2ODEyNDAsNjMwNTM0Nzc4LDQzNjM4OTc5Niw0MjQ0MjY2M119
