...
int main() {
    ...
    // Equivalent to !$omp parallel and !$omp end parallel:
    // Fortran:
    // !$omp parallel
    //   ...code...
    // !$omp end parallel
    ...
    // Fortran sets a, b, c as private.
    // In C++, we must have these variables declared.
    // Example: 
    int a = 0, b = 1, c = 2; 
    #pragma omp parallel private(a, b, c)
    {
        ...
        // unless specified otherwise or if they are declared inside this block.
        // For clarity, re-declare inside the block if needed:
        a = 10; b = 20; c = 30; 
    }

    // Equivalent to !$omp parallel default(none) shared(p, q, r):
    // Fortran requires that no variable is assumed to have a default sharing attribute.
    // p, q, r must be explicitly listed as shared.
    ...
    // Let's re-declare a, b, c outside:
    a = 1; b = 2; c = 3;
    #pragma omp parallel default(private) shared(a, b, c)
    {
        // Here a, b, c are shared. Any other variable is private by default.
        // If we use a variable not specified as shared, it becomes private.
        // Each thread sees the same a, b, c.
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTMzNTYzNTA5NSwtNDU2NzUyOTgwLDE3ND
