...
在此例中，任意时刻只有一个名为 `dataupdate` 的 `critical` 区域可以被执行，并且只有一个线程可以执行该区域。  
也就是说，`reorganize` 和 `reorganize_again` 函数不能同时被调用，并且这些函数的多次调用不能同时处于活动状态。（除非有其他未受 `critical` 构造保护的调用存在。）

**注意：**  
`critical` 区域的名称在整个程序中都是全局的（无论是否跨模块边界）。  
因此，如果在多个模块中定义了相同名称的 `critical` 区域，这些区域也不能同时被执行。

如果需要类似局部互斥锁的功能，请参阅后续内容。
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwNjMwNzA5NjgsLTk4MTQ4NzY4NywtMT
A0MTI5MDc3NCwtNDcwMDI1NzY1LC0xMTM4OTAzMjE0LDkwMzg5
MDk5MywtMjI2NTE3NzA4LC0zNDkwNDgzNCw0MTY1ODgwMzYsNj
cxNDMzMTEwLDgzMjg0NTE2OCwxMDcwNzkyNDMzLDEwOTE0NDEz
NzksLTU1NDYxMjQ4MywtNTA0NjMwMjM5LC0xMTkwOTc4NDE1LC
02NzgzNDgwNjQsLTkxNTk0ODcxMywtMTc4ODk0NDc4NiwtNjg4
NDU5NjYxXX0=
-->