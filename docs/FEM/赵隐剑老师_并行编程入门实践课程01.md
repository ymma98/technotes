...
**`critical` 构造**限制了关联语句/代码块的执行，使得在任何时间点只有一个线程可以执行该代码块。

`critical` 构造可以选择性地包含一个全局名称，用于标识该 `critical` 构造的类型。同一时间内，两个线程不能同时执行具有相同名称的 `critical` 构造。
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5OTI4NDAxMjgsLTk4MTQ4NzY4NywtMT
