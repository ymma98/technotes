...
$$  
B_r = -\frac{\partial A_\theta}{\partial z} \\ B_z = \frac{1}{r} \frac{\partial \left( rA_\theta \right)}{\partial r}  
$$
...
$$  
\begin{aligned} \vec{B} &= B_\theta \hat{e}_\theta + \vec{B}_p \\ &= B_\theta \hat{e}_\theta + \frac{1}{r} \nabla \psi \times \hat{e}_\theta \end{aligned}  
$$
...
$$  
\begin{aligned} \mu_0 J_\theta &= \left( \nabla \times \vec{B}_p \right)_\theta = -\frac{1}{r} \Delta^* \psi \end{aligned}  
$$
...
$$  
\begin{aligned} & \Delta^* \Psi = -\mu_0 r^2 P^\prime - FF^\prime \\ & \Delta^* = r \frac{\partial}{\partial r} \left( \frac{1}{r} \frac{\partial }{\partial r} \right) + \frac{\partial^2 }{\partial z^2 } \end{aligned}  
$$
...
$$  
\begin{aligned} & \Delta^* \Psi = -\mu_0 r^2 P^\prime, \quad F = 0 \\ & \Delta^* \Psi = -\mu_0 r J_\theta \\ & J_\theta = r P^\prime \end{aligned}  
$$
...
$$  
\begin{aligned} \Delta^* \psi &\equiv r \frac{\partial}{\partial r} \Bigl(\frac{1}{r}\,\frac{\partial \psi}{\partial r}\Bigr)+ \frac{\partial^2 \psi}{\partial z^2} = -\mu_0\,r^2\,p'(\psi),\\ J_\theta &= r\,p'(\psi),\\ B_z &= \frac{1}{r}\,\frac{\partial\psi}{\partial r}, \qquad B_r = -\frac{1}{r}\,\frac{\partial\psi}{\partial z}. \end{aligned}  
$$
...
$$  
\begin{aligned} \int \nabla\cdot (\frac{1}{r^2} \nabla \psi) v dV &= -\mu_0 \int p^\prime v dV \\ \int \nabla \cdot (v\frac{1}{r^2} \nabla \psi) dV - \int \frac{1}{r^2} \nabla \psi \cdot \nabla v dV &= -\mu_0 \int p^\prime v dV \\ \int v\frac{1}{r^2} \nabla \psi \cdot\vec{n} dS - 2\pi\int \frac{1}{r} (\frac{\partial \psi}{\partial r}\frac{\partial v}{\partial r} + \frac{\partial \psi}{\partial z} \frac{\partial v}{\partial z}) drdz &= -\mu_0 2\pi \int rp^\prime v drdz \\ \int v \frac{1}{r} \frac{\partial \psi}{\partial r} n_r dz + \int v \frac{1}{r} \frac{\partial \psi}{\partial z} n_z dr - \int \frac{1}{r} (\frac{\partial \psi}{\partial r}\frac{\partial v}{\partial r} + \frac{\partial \psi}{\partial z} \frac{\partial v}{\partial z}) drdz &= -\mu_0 \int rp^\prime v drdz \\ \int \frac{1}{r} (\frac{\partial \psi}{\partial r}\frac{\partial v}{\partial r} + \frac{\partial \psi}{\partial z} \frac{\partial v}{\partial z}) drdz &= \int v \frac{1}{r} \frac{\partial \psi}{\partial r} n_r dz + \int v \frac{1}{r} \frac{\partial \psi}{\partial z} n_z dr +\mu_0 \int rp^\prime v drdz \end{aligned}  
$$

  

其中 n_r 和 n_z 是垂直于表面的单位向量, 对于矩形网格, r=r_w=r_{max} 面/线, n_r=1, r=0 处 n_r=-1. z=z_{max} 处 n_z=1, z=z_{min}处n_z=-1. test function v 在固定边界条件处取 0, 在 Neumann BC 上可以取任意值。
...
应用边界条件, r\to0, \psi=0 得到 c_2=0. 在边界处, 有 \psi=\psi_w, 且在 r=r_s处, 有 \psi=0, 得到:

$$  
\begin{cases} c_1 = \frac{1}{8} \mu_0 C r_s^2 \\ c_3 = \frac{\psi_w}{r_w^2 - r_s^2} \\ c_4 = -c_3 r_s^2 = - \frac{\psi_w}{r_w^2 - r_s^2} r_s^2 \end{cases}  
$$
...
$$  
\begin{aligned} -\frac{1}{2} \mu_0 C r_s^3 + 2c_1r_s &= 2c_3r_s \\ c_3 &= -\frac{1}{4}\mu_0 C r_s^2 + c_1 = -\frac{1}{8} \mu_0 C r_s^2 \end{aligned}  
$$
...
$$  
\begin{aligned} -\frac{1}{8} \mu_0 C r_s^2 &= \frac{\psi_w}{r_w^2 - r_s^2} \\ x_s^2 (1-x_s^2) = -\frac{8\psi_w}{\mu_0 C r_w^4} \end{aligned}  
$$
...
![输入图片说明](file:///E:/garnet/%E6%89%BE%E5%B7%A5%E4%BD%9C/trash/figs/image.3rbkw6dkpa.webp?lastModify=1757335213)
...
$$  
\begin{cases} p_0(\psi) = \cdots \quad \text{where} \quad \psi<0 \\ p_0(\psi) = p_{open} \quad \text{where} \quad \psi \geq 0 \end{cases}  
$$
...
-   判断, 是否需要限定 S?
    ...
    -   如果需要限定 S, 则第 n 次迭代完成后, 更新 C, 有 C_{n+1}=C_{n}\times S/S_n, 也就是说, 如果 S_n 比设定的值S大了, 那么下次迭代就降低 C. 因此, 这里也需要一个程序计算每一步的 S_n, 计算方法是, 找到每一步 \psi=0 的 contour, 然后计算该 contour 包围的面积
        
    -   数据后处理, 输出数据为 .csv 格式, 将数据输出到一个均匀网格上
        

目前想到的迭代方法: S 是设置的 separatrix 的面积。初始, p(\psi)=Cp_0(\psi)。迭代完成之后, 得到 S_n, n 代表是第n次迭代, C_{n+1} = C_n \frac{S}{S_n}, p_{n+1} = C_{n+1}p_n, 也就是说, 如果 S_n 比设定的值大了, 那么下次迭代就降低压强。
...
$$  
\psi(r, z)= -r A_\phi=\frac{\psi_0}{R_0^4}\left\{\left(r^2-R_0^2\right)^2+\frac{z^2}{E^2}\left(r^2-R_x^2\right)\right. \\ \left.-\tau R_0^2\left[r^2 \ln \left(\frac{r^2}{R_0^2}\right)-\left(r^2-R_0^2\right)-\frac{\left(r^2-R_0^2\right)^2}{2 R_0^2}\right]\right\},  
$$
...
其中 R_0 是 r_o, 并且有 \psi(R_0,0)=0。R_x 是 X 点半径, \tau是三角度 triangularity, E 是拉长比。

$$  
\begin{aligned} B_r= & -\frac{1}{r} \frac{\partial \psi}{\partial z}=-\frac{2 \psi_0}{r R_0^4}\left[\frac{z}{E^2}\left(r^2-R_x^2\right)\right], \\ B_z= & \frac{1}{r} \frac{\partial \psi}{\partial r}=\frac{2 \psi_0}{R_0^4}\left\{2\left(r^2-R_0^2\right)+\frac{z^2}{E^2}\right. \\ & \left.-\tau R_0^2\left[\ln \left(\frac{r^2}{R_0^2}\right)-\frac{\left(r^2-R_0^2\right)}{R_0^2}\right]\right\} . \end{aligned}  
$$
...
$$  
\psi = 4\,\psi_0\,\epsilon^2,\quad \epsilon \equiv \frac{r - R_0}{R_0} \ll 1,\quad \kappa \equiv \frac{2E}{\sqrt{1 - R_x^2/R_0^2}}.  
$$
...
$$  
B_p = 8\,\frac{\psi_0}{R_0^2}\,\epsilon,\qquad q_0 = \frac{r\,B_0}{R_0\,B_p} = \frac{B_0\,R_0^2}{8\,\psi_0},  
$$
...
$$  
\begin{aligned} \psi(r, z)= \frac{\psi_0}{R_0^4}\left\{\left(r^2-R_0^2\right)^2+\frac{z^2}{E^2}r^2\right\}, \end{aligned}  
$$
...
$$  
\begin{aligned} -\mu_0 r^2 p^\prime &= \frac{2(1+4E^2) \psi_0 r^2}{E^2 R_0^4} \\ p^\prime &= - \frac{2(1+4E^2) \psi_0}{\mu_0 E^2 R_0^4} \end{aligned}  
$$

  

该 FRC 模型也适用于镜场构型，只需进一步令 R_0^2<0。

## 程序实现

├── external │ └── CLI11.hpp ├── include │ ├── mesh.hpp │ ├── parameters.hpp │ └── solver.hpp ├── src │ ├── mesh.cpp │ ├── parameters.cpp │ └── solver.cpp
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTM3MDA0MDI5NywxMzQ4MTQ3NTg3LC0xND
