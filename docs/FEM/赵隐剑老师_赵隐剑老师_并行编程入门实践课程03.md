...
int main()
{
    ...
    int p1 = 3, p2 = 7;
    double c = 0.5;
    int mpi_n = 2;     // Suppose 2 MPI processes

    // Allocate p as a vector of n arrays, each with 9 doubles
    std::vector<std::array<double, 9>> p(n);
    // Initialize p with some values (for demonstration)
    for (int i = 0; i < n; ++i) {
        for (int row = 0; row < 9; ++row) {
            p[i][row] = 1.0 * (i+1 + row);
        }
    ...
    // Call our sub_ppl function
    sub_ppl(n, p1, p2, p, c, mpi_n);

    // Print out part of p to see updated values
    for (int i = 0; i < n; ++i) {
        std::cout << "p[" << i << "]: ";
        for (int row = 0; row < 9; ++row) {
            std::cout << p[i][row] << " ";
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2NzYyNjkzOCw0NjM1ODQzNiwzNTI2Nj
